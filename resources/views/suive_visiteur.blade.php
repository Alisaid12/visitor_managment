...
@section('content')
    <form action="{{ route('visit.store', ['id' => $visit->id, 'visiteur_id' => $visit->visiteur->id]) }}" method="POST">
        ...
        {{-- <div class="form-group mb-2"> <label for="date_visit_prochaine" class="mt-3">Date de prochaine visite :</label>
            <input type="date" class=" @error('date_visit_prochaine') is-invalid @enderror" id="date_visit_prochaine"
                name="date_visit_prochaine" value="{{ old('date_visit_prochaine') }} "> @error('date_visit_prochaine')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        </div> --}}
        <div class="form-group mb-2">
            <label for="date_visit_prochaine" class="mt-3">Date et heure de la prochaine visite :</label>
            <input type="datetime-local" class="   @error('date_visit_prochaine') is-invalid @enderror"
                id="date_visit_prochaine" name="date_visit_prochaine" value="{{ old('date_visit_prochaine') }} "
                style="width: 259px;height:34px;">
