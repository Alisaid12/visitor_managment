...
@section('content')
    <div class="table-responsive">
        <h3 class="mb-3"><u>Liste des visiteurs:</u></h3>
        <a type="button" class="btn btn-success mb-2" href="{{ url('/visiteur/create') }}">Ajouter un visiteur</a>
        ...
        <tbody>
             ...
             <tr>
               ...
               <td>{{$item->ville}}</td>
                 ...
                 <td>{{ \App\Models\User::find($item->responsable_id)->name }}</td>
                 ...
                 <td>
                     <a href="{{url('/visiteur/' .$item->id)}}" title="View Visiteur">
                        <button class="btn btn-info mb-1"><i class="fa fa-eye" aria-hidden="true"></i>Info</button>
                     </a>
                    <a href="{{url('/visiteur/' .$item->id. '/edit')}}" title="Modifier Visiteur">
                         <button class="btn btn-primary mb-1" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Modifier</button>
                    </a>
                     <form action="{{ route('visiteur.destroy', $item->id) }}" method="POST">
                         @csrf
                         @method('DELETE')
                         <button type="button" class=" class="btn btn-danger btn-sm"" data-bs-toggle="modal" data-bs-target="#delete-modal-{{ $item->id }}">
                             <i class="fa fa-trash-o" aria-hidden="true"></i> Supprimer/                         </button>
                         ...
                         <div class="modal fade" id="delete-modal-{{ $item->id }}" tabindex="-1" aria-labelledby="delete-modal-{{ $item->id }}-label" aria-hidden="true">
                             <div class="modal-dialog">
                                 <div class="modal-content">
                                     <div class="modal-header">
                                         <h5 class="modal-title" id="delete-modal-{{ $item->id }}-label">Confirmation de suppression</h5>
